...
function fetchedRoleBApproveLeadList(data) {
    return {
        type: types.ROLEB_FETCH_APPROVE_LEAD_LIST,
        data
    };
}

function fetchingRoleBApproveLeadList(data) {
    return {
        type: types.ROLEB_FETCHING_APPROVE_LEAD_LIST,
        data
    };
}

export function fetchRoleBApproveLeadList(page: number, navigator: any, refresh: boolean = false) {
    return (dispatch) => {
        dispatch(fetchingRoleBApproveLeadList({ refresh }));
        let items: ILead[] = [{
            ID: "aaa",
            ObjectID: "ccc",
            FirstTouch: "易车网",
        ...
        //return setTimeout(() => dispatch(fetchedRoleBApproveLeadList({ data, refresh })), 2000);
        return dispatch(fetchedRoleBApproveLeadList({ data, refresh }));
    };
}
